...
def get_data(iss, observer, ts, config):
    ...
    return {
        ...
        "az": az.degrees,
        "shift_raw": shift_khz,
        "shift_khz": rounded_shift_khz,
        "tuned_freq": tuned_freq_mhz,
        "uplink_mhz": config.get('uplink_frequency_mhz')
    }

def run_curses(stdscr, iss, observer, ts, config, sound_mode, ser, is_demo=False):
    curses.curs_set(0)
    ...
    last_led1_color = None
    
    # Mapping index to Hex RGB intensity strings
    HEX_MAP = {
        0: "000", # Off
        1: "00f", # Blue
        2: "0f0", # Green
    ...
    while True:
        ...
        if is_demo:
            ...
            # Simulate a 60s pass: Elevation 0->45->0, Doppler +15->-15
            alt_val = 45.0 * (1 - ((elapsed - 30) / 30)**2)
            shift_raw = 15.0 - (elapsed * 0.5)
            rounded_shift = round(shift_raw / step_khz) * step_khz
            ...
            data = {
                ...
                "az": (elapsed * 6) % 360,
                "shift_raw": shift_raw,
                "shift_khz": rounded_shift,
                "tuned_freq": config['base_frequency_mhz'] + (rounded_shift / 1000.0),
                "uplink_mhz": config.get('uplink_frequency_mhz')
            }
        else:
        ...
        was_above_horizon = is_above or (is_demo and data['alt'] > 0)
        ...
        if ser:
            try:
                ...
                alt_val = data['alt']
                el_val = max(0, int(round(alt_val)))
                cmd = f"azel{az_val:03d},{el_val:02d}\n"
                ser.write(cmd.encode())

                # Doppler LED Logic
                current_led1_color = 0 # Default Off (Below horizon)
                if alt_val > 0:
                    shift = data['shift_khz']
                    if shift >= 10: current_led1_color = 5      # Magenta (M)
                    elif shift >= 5: current_led1_color = 1     # Blue (B)
                    elif shift <= -10: current_led1_color = 4    # Red (R)
                    elif shift <= -5: current_led1_color = 6     # Yellow (Y)
                    else: current_led1_color = 2                # Green (G - Nominal)
                
                if current_led1_color != last_led1_color:
                    hex_val = HEX_MAP.get(current_led1_color, "000")
                    led_cmd = f"colr{hex_val}\n"
                    ser.write(led_cmd.encode())
                    last_led1_color = current_led1_color
            except Exception:
                pass # Silently fail to keep the UI responsive

        title = f"--- {sat_name} LIVE TRACKER ({step_khz}kHz Steps) ---"
        if is_demo:
            title = f"--- DEMO MODE: {sat_name} (60s simulation) ---"
...
def main():
    ...
    parser.add_argument("-demo", action="store_true", help="Simulate a 60-second satellite pass for LED testing")
    ...
    if args.demo:
        config['base_frequency_mhz'] = 437.800
        config['tuning_increment'] = 5.0
        # Dummy satellite object
        class MockSat: name = "ISS (SIMULATED)"
        ...
        curses.wrapper(run_curses, MockSat(), None, None, config, args.sound, ser, True)
        if ser: ser.close()
        return
