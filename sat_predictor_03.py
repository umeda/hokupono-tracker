...
def run_prediction(sat, observer, ts, config):
...
def main():
    ...
    for entry in to_predict:
        ...
        if sat:
            all_passes.extend(run_prediction(sat, observer, ts, config))
    ...
    for p in all_passes:
        ...
        print(f"\n{date_str} | Satellite: {p['name']} ({type_str}, {orbit_str}, {icon_display}) - Quality: {p['quality']}")
        if p['quality'] == "low":
