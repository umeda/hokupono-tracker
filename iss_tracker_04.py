...
def run_curses(stdscr, iss, observer, ts, config):
    curses.curs_set(0)
...
def main():
    ...
    parser.add_argument("-mode", choices=["curses"], required=True)
    ...
    if args.mode == "curses":
        curses.wrapper(run_curses, selected_sat, observer, ts, config)
    else:
        # Standard one-shot
        data = get_data(selected_sat, observer, ts, config)
